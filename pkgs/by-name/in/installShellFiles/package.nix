...
makeSetupHook {
  ...
  substitutions = {
    ...
    emulatorAvailable = if hostPlatform_ != null then lib.debug.traceValFn (v: "emulatorAvailable ${builtins.toString v}") (if hostPlatform_.emulatorAvailable buildPackages && allowEmulator then 1 else 0) else 0;
