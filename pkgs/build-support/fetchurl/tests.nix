...
in
{
  ...
  # Tests that custom mirror names can be used and resolved via NIX_MIRRORS_* environment variables.
  # Note that we set the env variable here via the nix derivation - this won't immediately work via an externally
  # set variable since these custom variables are not added to impureEnvVars.
  urls-env-mirrors =
    let
