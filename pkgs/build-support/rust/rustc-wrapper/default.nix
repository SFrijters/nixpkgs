...
let
  ...
  rustc-wrapper = replaceVars ./rustc-wrapper.sh {
    inherit shell sysrootFlag defaultArgs;
    prog = "${lib.getExe' rustc-unwrapped "rustc"}";
    extraFlagsVar = "NIX_RUSTFLAGS";
  };

  rustdoc-wrapper = replaceVars ./rustc-wrapper.sh {
    inherit shell sysrootFlag defaultArgs;
    prog = "${lib.getExe' rustc-unwrapped "rustdoc"}";
    extraFlagsVar = "NIX_RUSTDOCFLAGS";
  };
in
runCommand "${rustc-unwrapped.pname}-wrapper-${rustc-unwrapped.version}"
  ...
  ''
    ...
    install -Dm755 ${rustc-wrapper} $out/bin/rustc
    install -Dm755 ${rustdoc-wrapper} $out/bin/rustdoc
